...
class My_image:
    def __init__(self, new_size, mask_size, theta, center, translate, top, im_blur):
    ...
    def base_process(self, basep):
        ...
        base_expand = Image.new(base_resize.mode, self.mask_size, (0, 0, 0))
        base_expand.paste(base_resize)
        base_rotate = base_expand.rotate(np.rad2deg(self.theta), center=self.center,
...
def face_exchange(base_array, to_path, predictor):
    ...
    process_set = My_image(
        new_size, mask_size, theta, center, translate, Image.open(to_path), im_blur
    )
