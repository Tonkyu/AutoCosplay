...
class My_image:
    ...
    def shape_process(self, shape_base, base_resize):
        ...
        base_expand = Image.new(base_resize.mode, mask_size, 0)
...
def face_exchange(base_array, to_path, predictor):
    """
    base 顔を切り抜かれる方のnumpy配列（pillowから生成された）
    to 顔を貼り付けられる方の画像のパス
    返り値 新しく生成した画像のnumpy配列（pillowから生成された），と次の処理用のクラス，全体ではタプル
    """
    #detector = dlib.get_frontal_face_detector()
    #predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")
...
if __name__ == "__main__":
    ...
    for i, frame in enumerate(container.decode(video=0)):
        if i == 0:
            img, process_set = face_exchange(frame.to_ndarray(), "lennon.jpeg", predictor)
            cv2.imshow("win_img", img)
            cv2.waitKey(0)
        if i > 0:
            img = process_set.base_process(Image.fromarray(frame.to_ndarray()))
            cv2.imshow("win_img", np.array(img))
            key = cv2.waitKey(0)
            if key == 27:
                break
